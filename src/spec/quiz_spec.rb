...
describe 'invalid name error' do
    it 'should print a message saying invalid name error' do
        name = ""
        expect(invalid_name(name)).to be raise InvalidNameError, "Name must not be empty"
    end
end
...
describe 'invalid input error' do
    ...
    it 'should raise an InvalidInputError' do
        expect(invalid_input(answer)).to eq raise InvalidInputError, "Please use 'true' or 'false' answers" 
    end
end 